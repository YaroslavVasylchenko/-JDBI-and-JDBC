...
public class ThekioskProcessesCode {
    ...
    public ThekioskProcessesCode(ConstructorParams testConstructorParams) {
    ...
    public static void main(String[] args) {
        ...
        // Create an instance of Thekiosk with custom values
        ConstructorParams params = new ConstructorParams(3002, 337, "2023-07-19 ", "2023-07-20", 1,"subscribe","subscription");

        ThekioskProcessesCode customVodStore = new ThekioskProcessesCode(params);
        customVodStore.compareDatabases();
        ...
        try (Connection billingConnection = DriverManager.getConnection(BILLING_URL);
            ...
            for (LocalDateTime segment : sortedSegments) {
                ...
                if (Math.abs(billingCount - etlTotalTransitions) >= threshold) {
                    ...
                    String billingKeysQuery = "SELECT UPPER(CONCAT_WS('-', SUBSTRING(MD5(CONCAT(bp.id, '|thekiosk|" + etlType + "|', bp.status)), 1, 8), SUBSTRING(MD5(CONCAT(bp.id, '|thekiosk|" + etlType + "|', bp.status)), 9, 4), SUBSTRING(MD5(CONCAT(bp.id, '|thekiosk|" + etlType + "|', bp.status)), 13, 4)," +
                            " SUBSTRING(MD5(CONCAT(bp.id, '|thekiosk|" + etlType + "|', bp.status)), 17, 4), SUBSTRING(MD5(CONCAT(bp.id, '|thekiosk|" + etlType + "|', bp.status)), 21))) AS id, bp.* " +
                            "FROM b_processes bp " +
                            "WHERE bp.carrier = " + billingCarrierId + " " +
